...
def main():
    ...
    print(f"Loading model: {MODEL_NAME} (70B) with 4-bit NF4 on SINGLE GPU...")
    ...
    try:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            device_map="auto",  # 单卡模式下，auto 会自动映射到当前可见的那张卡
            token=HF_TOKEN,
            trust_remote_code=True
        )
    ...
    for combo in tqdm(target_combos, desc="Generating (Llama 70B NF4)"):
        ...
        while jokes_needed > 0:
            ...
            for joke in parsed_jokes:
                writer.writerow([combo, user_prompt, joke, "Llama-3.1-70B-NF4"])
            f_out.flush()

...
INPUT_CSV = BASE_DIR / "outputs" / "deepseek_jokes_raw.csv"
OUTPUT_DIR = BASE_DIR / "outputs"
OUTPUT_CSV = OUTPUT_DIR / "llm_jokes_cleaned.csv"
...
def main():
    ...
    with open(INPUT_CSV, newline="", encoding="utf-8") as handle:
        ...
        if "joke" in fieldnames:
            text_field = "joke"
        elif "raw_joke" in fieldnames:
            text_field = "raw_joke"
        else:
            raise ValueError("Input CSV missing required column 'joke' or 'raw_joke'.")
        theme_field = "theme" if "theme" in fieldnames else None

        for row in reader:
            total_rows += 1
            joke_raw = row.get(text_field, "")
            raw_text = "" if joke_raw is None else str(joke_raw)
            theme_raw = row.get(theme_field, "") if theme_field else ""
            theme_text = "" if theme_raw is None else str(theme_raw)
            ...
            rows.append((sid, joke_cleaned, theme_text))

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    with open(OUTPUT_CSV, "w", newline="", encoding="utf-8") as handle:
        writer = csv.writer(handle)
        writer.writerow(["rid", "stable_id", "joke_cleaned", "theme"])
        rows.sort(key=lambda item: item[0])
        for rid, (sid, joke, theme) in enumerate(rows, start=1):
            writer.writerow([rid, sid, joke, theme])
    ...
    Skipped (too short < 10): 0
    Skipped (too long > 1000): 0
    Skipped (duplicate normalized joke): 85955
